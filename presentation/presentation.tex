...
airtraffic install   (sudo / Admin)
  ...
  \textbf{Commands:}
  \begin{itemize}
    \item \texttt{airtraffic live} -- Shows real-time network usage
    \item \texttt{airtraffic since today | month} -- Shows historical usage
    \item \texttt{sudo airtraffic block <app>} -- Blocks app from network
    \item \texttt{sudo airtraffic allow <app>} -- Allows app to use network
    \item \texttt{airtraffic blocked | allowed} -- Lists blocked/allowed apps
    \item \texttt{airtraffic uninstall} -- Removes background service
  \end{itemize}
\end{frame}
...
\begin{frame}{Summary}
  \begin{itemize}
    \item AirTraffic lets you see which apps use your internet and control their access - all from one simple tool.
    \item A background service saves your network history so you can check past usage anytime.
    \item Works on all platforms: Windows blocks apps using its firewall; Mac/Linux stop apps from running.
    \item One-time setup makes the background service start automatically when you turn on your computer.
  \end{itemize}
\end{frame}
